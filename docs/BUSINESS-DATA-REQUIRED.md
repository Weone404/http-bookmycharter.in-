...
|---|---|---|---|
| ~~**B1**~~ | ~~Does Book My Charter **operate** aircraft, or **arrange** charter, or both?~~ | **ANSWERED 2026-09-22: both.** `BUSINESS_MODEL = 'hybrid'`. | Resolved |
| **B1a** | The operating permit / AOC number, and the registration of every aircraft Book My Charter operates itself. | `hybrid` still renders the arranged voice everywhere, because "our fleet", "our pilots" and an AOC number are externally checkable claims. Without the permit number and the registrations they cannot be published, so the operated half of the business is currently invisible on the site. Supplying these flips `mayClaimOwnInventory` and `mayClaimOwnCertification` and lets operated aircraft be described as operated. | |
| ~~**I7**~~ | ~~Where should a charter request be delivered?~~ | **ANSWERED 2026-09-22: the We One Aviation CRM.** `crmAdapter` is implemented and tested end to end against a local sink: auth header sent, flat JSON body delivered, reference returned. | Resolved |
| **I7a** | The CRM endpoint URL that should receive the POST, and the auth header it expects. | Set `CHARTER_CRM_URL`, and `CHARTER_CRM_AUTH` (plus `CHARTER_CRM_AUTH_HEADER` if it is not `authorization`). Until `CHARTER_CRM_URL` is set the API still returns 503 and the form tells people to phone. **A live site with an unconnected form loses every request.** | |
