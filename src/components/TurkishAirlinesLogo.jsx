...
export const TurkishAirlinesLogo = ({
  ...
  return (
    <div className={`flex items-center gap-3 select-none cursor-pointer ${className}`}>
      ...
      <div className="relative w-8 h-8 md:w-9 md:h-9 rounded-full bg-[var(--brand-primary)] flex items-center justify-center shadow-sm shrink-0">
        <svg
          viewBox="0 0 100 100"
          className="w-5 h-5 md:w-6 md:h-6 text-[var(--text-inverse)] fill-current transform rotate-[-8deg]"
        >
      ...
      {showText && (
        <span className="font-extrabold tracking-[0.18em] text-[15px] md:text-[17px] text-[var(--text-inverse)] uppercase font-sans">
          TURKISH AIRLINES
        </span>
      )}
    </div>
