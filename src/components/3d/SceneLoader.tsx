 ...
 */
export function SceneLoader({ inline = false }: { inline?: boolean }) {
  ...
  return (
    <div
      aria-hidden="true"
      className={`${
        inline ? 'absolute' : 'absolute'
      } inset-0 flex items-center justify-center bg-[var(--color-midnight)] transition-opacity duration-500 ${
        active ? 'opacity-100' : 'pointer-events-none opacity-0'
      }`}
    >
      <div className="w-[min(18rem,60vw)] text-center">
        <p className="text-[length:var(--text-micro)] uppercase tracking-[0.22em] text-[var(--color-ink-inverse-muted)]">
          Preparing the fleet
        </p>
        <div className="mt-4 h-px w-full overflow-hidden bg-white/15">
          <div
            className="h-px bg-[var(--color-cyan-accent)] transition-[width] duration-300 ease-out"
